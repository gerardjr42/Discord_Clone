...
// Functions for managing friend relationships between users
// Includes queries for listing pending and accepted friend requests
// Uses authenticated queries to ensure security
...
// Create a friend request for a user by their username
// Validates that the user exists and is not the current user
// Throws errors if user not found or is the current user
...
// Update the status of a friend request to either accepted or rejected
// Validates that the user is authorized to update the request
// Throws error if friend request not found or user unauthorized
...
// Helper function to map an array of items containing user IDs to include the full user objects
// Takes a QueryCtx, array of items T[], and key K that specifies which field contains the user ID
// Returns a new array with the user objects merged into each item
