...
//fetch a list of all direct messages for a user
...
// Return all direct messages for a user
  ...
  handler: async (ctx, { username }) => {
    ...
    const directMessage = directMessagesForCurrentUser.find((dm) =>
      directMessagesForOtherUser.find(
        (otherDm) => otherDm.directMessage === dm.directMessage
      )
    );
    if (directMessage) {
...
const getDirectMessage = async (
  ...
  if (!otherMember) {
    throw new Error("Direct message has no other usr");
  }
